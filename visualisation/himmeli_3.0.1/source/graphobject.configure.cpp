...
GraphObject::configure() {
  ...
  if((label_mode != "on") && (label_mode != "off") && (label_mode != "edge"))
    label_mode = "vertex";
  legend_mode = (cfg["LegendMode"][1] == "on");
  tree_mode = (cfg["TreeMode"][1] == "on");
  ...
  distance_unit = 1.0;
  if(label_mode == "on") distance_unit *= 1.5;
  ...
  if(verbose_mode) {
    ...
    if(legend_mode) printf("\t%-22s %-16s\n", "LegendMode", "on");
    else printf("\t%-22s %-16s\n", "LegendMode", "off");
    printf("\t%-22s %.0fs\n", "TimeLimit", time_limit);
    if(tree_mode) printf("\t%-22s %-16s\n", "TreeMode", "on");
    else printf("\t%-22s %-16s\n", "TreeMode", "off");
  }
  ...
  loci.push_back(Locus("VertexZVariable", cfg["VertexZVariable"]));
