...
static void* init(vector<int>&, vector<vector<Link> >&, vector<string>&);

/*
 *
 */
vector<Component>
Component::create(const std::vector<Edge>& graph) {
  unsigned int i;
  ...
  for(i = 0; i < names.size(); i++) {
    ...
    c.buffer = init(visits, inputs, names);  
...
init(vector<int>& subgraph, vector<vector<Link> >& inputs,
  ...
  if(subgraph.size() == names.size()) {
    ...
    return new ComponentObject(links, names);
  } 
  ...
  return new ComponentObject(links, labels);
}
